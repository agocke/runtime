...
and the diagnostic leaves) and assert it does **not** call the deferred `relocate_phase` /
`compact_phase` / `make_free_lists` / `fix_generation_bounds` / `recover_saved_pinned_info` tail,
confirm `get_region_mark_list` is heap-parameterized and calls `binary_search`, and verify the added
diagnostic leaves are true no-ops. No collection routes this driver. The plan-phase joins past the
relocate/compact/sweep boundary stay deferred: `gc_join_rearrange_segs_compaction` is
`#ifndef USE_REGIONS` and does not exist in this configuration, and `gc_join_adjust_handle_age_compact`
/ `gc_join_adjust_handle_age_sweep` follow the deferred `relocate_phase` / `compact_phase` /
`make_free_lists` execution — as do the deferred `fix_generation_bounds` invocation,
`recover_saved_pinned_info`, `GcPromotionsGranted` / `GcDemote`, the pinned-gap threading, and
`clear_gen1_cards`. Routing a collection entry point onto `plan_phase` (and translating that
relocate / compact / sweep execution) is the remaining work.
...
No collection routes this driver. Routing a collection entry point onto `plan_phase` / `relocate_phase`
(and translating the server `compact_phase` / `make_free_lists` execution with its
`gc_join_relocate_phase_done` / `gc_join_adjust_handle_age_*` joins, `recover_saved_pinned_info`,
`fix_generation_bounds` invocation, and `GcPromotionsGranted` / `GcDemote`) is the remaining work.
