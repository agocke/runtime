...
*sweep* leaf `sweep_uoh_objects` — the driver's fallback for a non-compacting LOH and for POH (which is
never compacted, so `plan_poh` is only this sweep) — is deferred to a separate slice: it mutates
`freeable_uoh_segment` (`PER_HEAP_FIELD_MAINTAINED`), whose per-heap conversion drags in the
`rearrange_uoh_segments` / `rearrange_small_heap_segments` / `delay_free_segments` segment-return family
and its `gc_heap*` threading through the hp-less WKS collection drivers (`garbage_collect_background`,
`gc1`). Still deferred, so no collection is routed: the `plan_phase` driver that sequences `plan_loh`
(its `settings.loh_compaction` / `loh_compacted_p` / `loh_compaction_requested` gating, the
`sweep_uoh_objects` fallback, and the plan-phase `gc_join`s), the `relocate_in_loh_compact` /
`compact_loh` execution, and the remaining SOH `fix_generation_bounds` / relocate / compact / sweep
execution.

The dedicated server smoke test (`src/tests/nativeaot/SmokeTests/ManagedGCServer`, which selects
`IlcManagedServerGC` and runs with `DOTNET_gcServer=1`) still cannot be built end-to-end because of a
pre-existing `src/tests` enumeration-infrastructure failure unrelated to the GC translation:
`src/tests/Common/dirs.proj` fails with `MSB4184: The expression "[System.IO.File]::ReadAllText('')"
cannot be evaluated. The value cannot be an empty string.` before any test project compiles. The
managed server GC assembly itself builds (`System.Private.GC.Server`) and links cleanly into
`clr.aot`; the executable server-collection path remains unrouted regardless.
