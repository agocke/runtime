...
`GcPromotionsGranted` / `GcDemote` / `thread_pinned_plug_gaps` / `clear_gen1_cards`) and assert it does
**not** call the deferred sweep tail `make_free_lists` / `recover_saved_pinned_info`,
confirm `get_region_mark_list` is heap-parameterized and calls `binary_search`, and verify the added
diagnostic leaves are true no-ops. No collection routes this driver. The compact-branch execution is
wired, but the sweep branch stays deferred: `gc_join_rearrange_segs_compaction` is
`#ifndef USE_REGIONS` and does not exist in this configuration, `gc_join_adjust_handle_age_sweep`
follows the deferred `make_free_lists` sweep execution, and the sweep-branch
`recover_saved_pinned_info` and `clear_gen1_cards` stay deferred with it. Routing a collection entry
point onto `plan_phase` (and translating the sweep branch's `make_free_lists` execution) is the
remaining work.
...
`fix_generation_bounds` invocation, and `GcPromotionsGranted` / `GcDemote`) now close in
`ManagedServerGCCompact.cs` and the wired `plan_phase` compact branch; routing a collection entry point
onto `plan_phase` / `relocate_phase` and translating the sweep branch's `make_free_lists` execution
(with `gc_join_adjust_handle_age_sweep`) is the remaining work.
