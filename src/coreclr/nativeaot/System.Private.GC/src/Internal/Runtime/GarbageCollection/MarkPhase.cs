...
internal unsafe partial struct gc_heap
{
    ...
    public static bool mark_phase_stack_roots(bool backgroundFinalMark = false)
    {
        ...
        sync_promoted_bytes(heap);
        if (settings.promotion == 0)
        {
            nuint promotion_threshold = 0;
            for (int gen_number = 0;
                 gen_number <= condemned_gen_number;
                 gen_number++)
            {
                promotion_threshold = unchecked(
                    promotion_threshold +
                    dynamic_data.dd_min_size(dynamic_data_of(heap, gen_number)) *
                    (nuint)(gen_number + 1) *
                    6 / 100);
            }

            int older_gen_number = Math.Min(
                condemned_gen_number + 1,
                GCInterfaceOffsets.max_generation);
            dynamic_data* older_dd = dynamic_data_of(heap, older_gen_number);
            nuint older_gen_size = unchecked(
                dynamic_data.dd_current_size(older_dd) +
                dynamic_data.dd_desired_allocation(older_dd) -
                (nuint)dynamic_data.dd_new_allocation(older_dd));
            nuint promoted = get_promoted_bytes(heap);
            settings.promotion =
                promotion_threshold > older_gen_size ||
                promoted > promotion_threshold
                    ? 1
                    : 0;
        }
